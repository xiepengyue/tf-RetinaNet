...
import retinanet2.resnet_utils as ru
...
@slim.add_arg_scope
def basicblock(inputs, depth, depth_bottleneck, stride,
               rate=1, scope=None):
    """Bottleneck residual unit variant with BN before convolutions
    When putting together 2 consecutive ResNet blocks that use this unit,
    one should use stride =2 in the last unit of first block
...
def resnet_v2_block(scope, base_depth, num_planes, stride, Basicblock=bottleneck):
    """
    Args:
    ...
    """
    return ru.Block(scope, Basicblock, [{
        'depth': base_depth * 4,
    ...
    with tf.variable_scope(scope, 'FPN', [inputs], reuse=reuse) as sc:
        with slim.arg_scope([slim.conv2d, bottleneck]):
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                block1 = resnet_v2_block('block1', base_depth=64, num_planes=num_planes[0], stride=1, Basicblock=Basicblock)
                c2 = stack_resnet_v2_units(mp1, block1)
                # block 2
                block2 = resnet_v2_block('block2', base_depth=128, num_planes=num_planes[1], stride=2, Basicblock=Basicblock)
                c3 = stack_resnet_v2_units(c2, block2)
                # block 3
                block3 = resnet_v2_block('block3', base_depth=256, num_planes=num_planes[2], stride=2, Basicblock=Basicblock)
                c4 = stack_resnet_v2_units(c3, block3)
                # block 4
                block4 = resnet_v2_block('block4', base_depth=512, num_planes=num_planes[3], stride=2, Basicblock=Basicblock)
...
def RetinaNet_FPN(inputs,
    ...
    feature_maps = []                    #Retina_FPN
    with tf.variable_scope(scope, 'resnet_v2_50', [inputs], reuse=reuse) as sc:
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, Basicblock]):
                with slim.arg_scope([slim.batch_norm], is_training=is_training):
                    c1 = ru.conv2d_same(inputs, 64, 7, stride=2, scope='conv1')
                    bn1 = slim.batch_norm(c1, scope='norm1', activation_fn=tf.nn.relu)
                    mp1 = slim.max_pool2d(bn1, [3, 3], stride=2, scope='pool1', padding='SAME')
...
FPN.default_image_size = 448  # shorter side in COCO image set  #512


def RetinaNet_FPN18(inputs,
          is_training=True,
          reuse=None,
          scope=None):
    num_planes = [2, 2, 2, 2]
    return RetinaNet_FPN(inputs,
               ...
               reuse=reuse,
               scope=scope)
RetinaNet_FPN18.default_image_size = FPN.default_image_size

def RetinaNet_FPN34(inputs,
          is_training=True,
          reuse=None,
          scope=None):
    num_planes = [2, 2, 2, 2]
    return RetinaNet_FPN(inputs,
               ...
               reuse=reuse,
               scope=scope)
RetinaNet_FPN34.default_image_size = FPN.default_image_size
